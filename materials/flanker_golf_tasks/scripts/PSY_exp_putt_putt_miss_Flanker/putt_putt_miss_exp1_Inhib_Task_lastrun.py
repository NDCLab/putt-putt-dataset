...
expName = 'multiEF'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/khoss005/Library/CloudStorage/OneDrive-FloridaInternationalUniversity/Psychopy/Sarah_Exp1_Flanker/Sarah_Exp1_EF/Sarah_exp1_Inhib_Task_lastrun.py',
...
while continueRoutine:
    ...
    if begin_keyResp.status == STARTED and not waitOnFlip:
        theseKeys = begin_keyResp.getKeys(keyList=['space'], waitRelease=False)
...
while continueRoutine:
    ...
    if begin_keyResp_4.status == STARTED and not waitOnFlip:
        theseKeys = begin_keyResp_4.getKeys(keyList=['space'], waitRelease=False)
...
while continueRoutine:
    ...
    if begin_keyResp_3a.status == STARTED and not waitOnFlip:
        theseKeys = begin_keyResp_3a.getKeys(keyList=['space'], waitRelease=False)
...
for thisBlockLoop in blockLoop:
    ...
    while continueRoutine:
        ...
        if pracOrMain_keyResp.status == STARTED and not waitOnFlip:
            theseKeys = pracOrMain_keyResp.getKeys(keyList=['space'], waitRelease=False)
    ...
    while continueRoutine:
        ...
        if pracOrMain_keyResp_3.status == STARTED and not waitOnFlip:
            theseKeys = pracOrMain_keyResp_3.getKeys(keyList=['z', 'm'], waitRelease=False)
    ...
    trialLoop = data.TrialHandler(nReps=numberReps, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('imageSelect.xlsx'),
    ...
    for thisTrialLoop in trialLoop:
        ...
        continueRoutine = True
        # update component parameters for each repeat
        if trialLoop.thisN == 0 or trialLoop.thisN % 16 != 0:
            continueRoutine = False
